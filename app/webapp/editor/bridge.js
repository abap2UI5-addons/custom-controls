...
(function () {
  ...
  function post(target, origin, type, payload) {
    ...
    try {
      ...
      console.error("editor bridge: postMessage failed:", e);
    }
  }
  ...
  window.addEventListener("message", function (event) {
    ...
    if (data.type === TYPE_LOAD) {
      if (typeof window.loadJsEditor !== "function") {
        console.error("editor bridge: editor.js did not load");
        return;
      }
      try {
        window.loadJsEditor(data.image || "", data.filename || "");
      } catch (e) {
        console.error("editor bridge: could not open the image:", e);
      }
      return;
    }
