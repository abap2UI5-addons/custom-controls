// summerHtmlImageMapCreator - HTML image map editor.
//
// Vendored third-party code. Author: Vladimir Zhukov (summerstyle),
// https://github.com/summerstyle/summerHtmlImageMapCreator
//
// Not taken from upstream directly: this file was recovered from
// abap2UI5-addons/js-libraries, where the same editor lived inside the ABAP
// class z2ui5_cl_cc_imagemapster (methods load_editor_js / _css / _html) as
// XML-escaped string literals. Extracting it back into real files is what
// makes it reviewable, cacheable and updatable at all - but it also means
// this copy reflects whatever upstream revision that addon vendored, which
// is not recorded anywhere. Treat upstream as the reference when updating.
//
// One block at the end of this file is ours and marked as such; everything
// else is as recovered.
...
loadJsEditor = function(base64_image, filename) {
 var summerHtmlImageMapCreator = (function() {
    ...
    /* ------------------------------------------------------------------
     * z2ui5cc patch - the ONLY change to the vendored editor.
     *
     * Upstream keeps `app` inside this IIFE, so a host page has no way to
     * ask the editor what the user drew. bridge.js needs exactly that: it
     * reads app.getHTMLCode() and posts the areas to the parent window.
     *
     * Everything else in this file is upstream, unmodified. Keep it that
     * way - re-extracting a newer upstream should mean re-applying this one
     * block, nothing else.
     * ---------------------------------------------------------------- */
    window.z2ui5ccEditorApp = app;
 })();
}