    ...
    return Control.extend("z2ui5_cci.cc.ImageMapster", {
      ...
      _options() {
        ...
        return {
          ...
          mapKey: "data-key",
          areas,
          // The selection is maintained HERE, from the event, and never read
          // back out of ImageMapster.
          //
          // mapster("get") reports the state as it was *before* the click,
          // because onClick runs ahead of the toggle - reading it made every
          // event lag one click behind. Deferring the read by a tick fixes
          // that in isolation but did not in the running app, so the
          // dependency on when mapster applies its own state is gone
          // altogether: data.key and data.selected already carry the NEW
          // state, synchronously and without a timer.
          //
          // The list keeps click order rather than area order; only its
          // contents are contractual.
          onClick: (data) => {
            const key = data.key;
            ...
            if (config.singleSelect) {
              ...
              list = Util.toList(this.getSelectedKeys());
              const at = list.indexOf(key);
              if (selected && at < 0) list.push(key);
              else if (!selected && at >= 0) list.splice(at, 1);
            }
